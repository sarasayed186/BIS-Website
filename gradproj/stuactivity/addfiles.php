...
if (isset($_GET['delete'])) {
  $ids = $_GET['delete'];
  $delete = "DELETE FROM `p47_stuactsimgs` WHERE id = $ids ";
...
if (isset($_POST['submit'])) {
  ...
  $allowTypes = array('jpg', 'png', 'jpeg', 'gif', 'mp4','MP4');
  ...
  if (!empty($fileNames)) {
    foreach ($_FILES['files']['name'] as $key => $val) {
      ...
      if (in_array($fileType, $allowTypes)) {
        ...
        if (move_uploaded_file($_FILES["files"]["tmp_name"][$key], $targetFilePath)) {
          // Image db insert sql 
          $evid = "SELECT * from `p47_studentacts` where `activityCode`=$id";
    ...
    if (!empty($insertValuesSQL)) {
      ...
      $insert = $bis->query("INSERT INTO `p47_stuactsimgs` (file_name, uploade_on,activityCode ,filetype) VALUES $insertValuesSQL");
...
$selectname = $bis->query("SELECT `activityName` FROM `p47_studentacts` WHERE `activityCode` =$id");
    ...
    <section class="gallerys-page back">
      <div class="container-lg">
        ...
        <div class="section-title">
          <div class="n-title"><?php echo $i['activityName'] ?></div>
        </div>
